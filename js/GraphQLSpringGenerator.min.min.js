...
`;return[{name:this.createSourceDirectoryFromArtefact(this.packageName)+"Application.java",content:e}]}generateApplicationPropertiesFile(e,t){return[{name:"src/main/resources/application.properties",content:`# Spring Boot application properties
# Server port
...
`}]}generateControllerFiles(){let e=this.model.entities,t=[];return e.forEach(e=>{let a=e.name,r=stringUtil.camelize(a),i=stringUtil.upperCamel(r),o=this.getPrimaryKeys(e);if(o.length>0){let n=o.map(e=>`@Argument ${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),s=o.map(e=>stringUtil.camelize(e.name)).join(", "),p=`package ${this.packageName}.controller;
...
public class ${i}Controller {
    ...
    @QueryMapping
    public ${i}Connection get${i}s(@Argument(name = "pageNumber") Integer pageNumber, @Argument(name = "pageSize") Integer pageSize, @Argument(name = "dataFilter") List<DataFilter> dataFilter, @Argument(name = "dataOrder") List<DataOrder> dataOrder) {
...
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`controller/${i}Controller.java`,content:p})}}),t}generateServiceFiles(){let e=this,t=this.model.entities,a=[];return t.forEach(t=>{let r=t.name,i=stringUtil.camelize(r),o=stringUtil.upperCamel(i),n=this.getPrimaryKeys(t),s="";if(t.columns.forEach(t=>{let a=e.getDataType(t.type),r=e.getFilterType(a),i=stringUtil.camelize(t.name);s+=`		this.fetchProperties.add("${i}", "${i}", "${a}", "${r}");\r
`}),n.length>0){let p=n.map(e=>`${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),l=n.map(e=>stringUtil.camelize(e.name)).join(", "),c='"'+n.map(e=>stringUtil.camelize(e.name)).join('", "')+'"',d=n.map(e=>stringUtil.upperCamel(stringUtil.camelize(e.name))).join("And"),g=[];n.forEach(e=>{let t=stringUtil.upperCamel(stringUtil.camelize(e.name));g.push(`input.get${t}() == null`)});let m=`package ${this.packageName}.service;
...
import ${this.packageName}.output.${o}Connection;
...
${s}
     ...
     */
    public ${o}Connection get${o}s(Integer pageNumber, Integer pageSize, List<DataFilter> dataFilter, List<DataOrder> dataOrder) {
    	List<DataOrder> filteredDataOrder = this.fetchProperties.filterFieldName(dataOrder);
        Page<${o}> page = ${i}Repository.findAll(
        ...
        return new ${o}Connection(page);
    }

    /**
     * Retrieves ${o} record matching the given primary key.
     *
     * @param ${n.map(e=>stringUtil.camelize(e.name)).join(", ")} the primary key value(s) used for filtering.
    ...
    public ${o} create${o}(${o}Input input) {
        ...
        return ${i}Repository.findOneBy${d}(
            ${n.map(e=>`saved.get${stringUtil.upperCamel(stringUtil.camelize(e.name))}()`).join(", ")}
        );
    }
     ...
     *
     * @param ${n.map(e=>stringUtil.camelize(e.name)).join(", ")} the primary key value(s) of the record to delete.
     */
    @Transactional
...
`;a.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`service/${o}Service.java`,content:m})}}),a}gereratePageUtilFile(){let e=`package ${this.packageName}.utils;
...
`;return[{name:this.createSourceDirectoryFromArtefact(this.packageName)+"utils/DataOrder.java",content:e}]}getNestedRelations(e,t,a="",r=2){if(r<=0)return[];let i=[],o=this.getPrimaryKeys(t).length>1;return t.columns.forEach(n=>{if(this.isForeignKey(e,t,n)&&!o){let s=stringUtil.camelize(n.name.replace(/_id$/i,"")),p=a?`${a}.${s}`:s;i.push(p);let l=e.find(e=>e.name.toLowerCase()===s.toLowerCase());l&&i.push(...this.getNestedRelations(e,l,p,r-1))}}),t.columns.forEach(t=>{if(o){let n=stringUtil.camelize(t.name.replace(/_id$/i,"")),s=a?`${a}.${n}`:n;i.push(s);let p=e.find(e=>e.name.toLowerCase()===n.toLowerCase());p&&i.push(...this.getNestedRelations(e,p,s,r-1))}}),i}generateConnectionFiles(){let e=this.model.entities,t=[];return e.forEach(e=>{let a=stringUtil.camelize(e.name),r=stringUtil.upperCamel(a),i=`package ${this.packageName}.output;
...
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`output/${r}Connection.java`,content:i})}),t}generateRepositoryFiles(){let e=this.model.entities,t=[],a=this;return e.forEach(r=>{let i=stringUtil.camelize(r.name),o=stringUtil.upperCamel(i),n=this.getPrimaryKeys(r);if(n.length>0){let s=n.length>1?`${o}Id`:this.getJavaType(n[0].type),p=n.map(e=>stringUtil.upperCamel(stringUtil.camelize(e.name))).join("And"),l=n.map(e=>`${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),c="";["String","Long","Integer","Double","Float","Boolean","Character","Byte","Short"].includes(s)||s.includes(".")||(c=`import ${this.packageName}.entity.${s};
`);let d=this.getNestedRelations(e,r,"",a.maxRelationDepth||3),g="";(d.length>0||n.length>1)&&(g=`    @EntityGraph(attributePaths = { ${d.map(e=>`"${e}"`).join(", ")} })
...
${g}    Page<${o}> findAll(Specification<${o}> specification, Pageable pageable);
}
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`repository/${o}Repository.java`,content:m});let u=`package ${this.packageName}.repository;
...
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`repository/${o}InputRepository.java`,content:u})}}),t}generateEntityFiles(){let e=this,t=this.model.entities,a=[];return t.forEach(t=>{let r=t.name,i=stringUtil.upperCamel(stringUtil.camelize(r)),o=stringUtil.snakeize(r),n=this.getPrimaryKeys(t),s=`package ${this.packageName}.entity;
...
`;let p=[];t.columns.forEach(a=>{let r=a.name,i=stringUtil.camelize(r),o=a.type,l=this.getJavaType(o),c=i.substring(0,i.length-2),d=stringUtil.upperCamel(c);(a.primaryKey&&(s+=`    @Id
...
`,p.push({name:c,type:d})):s+=`    @Column(name = "${r}")
    private ${l} ${i};

`,n.length>1&&a.primaryKey&&!a.autoIncrement&&(s+=`    @ManyToOne
