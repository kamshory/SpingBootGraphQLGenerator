...
`;r.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`service/${n}Service.java`,content:u})}}),r}gereratePageUtilFile(){let e=`package ${this.packageName}.utils;
...
`;return[{name:this.createSourceDirectoryFromArtefact(this.packageName)+"utils/DataOrder.java",content:e}]}getNestedRelations(e,t,r="",a=2){if(a<=0)return[];let i=[],n=this.getPrimaryKeys(t).length>1;return t.columns.forEach(o=>{if(this.isForeignKey(e,t,o)&&!n){let s=stringUtil.camelize(o.name.replace(/_id$/i,"")),p=r?`${r}.${s}`:s;i.push(p);let l=e.find(e=>e.name.toLowerCase()===s.toLowerCase());l&&i.push(...this.getNestedRelations(e,l,p,a-1))}}),t.columns.forEach(t=>{if(n){let o=stringUtil.camelize(t.name.replace(/_id$/i,"")),s=r?`${r}.${o}`:o;i.push(s);let p=e.find(e=>e.name.toLowerCase()===o.toLowerCase());p&&i.push(...this.getNestedRelations(e,p,s,a-1))}}),i}generateConnectionFiles(){let e=this.model.entities,t=[];return e.forEach(e=>{let r=stringUtil.camelize(e.name),a=stringUtil.upperCamel(r),i=`package ${this.packageName}.output;
...
`;let p=[];t.columns.forEach(r=>{let a=r.name,i=stringUtil.camelize(a),n=r.type,l=this.getJavaType(n),c=i.substring(0,i.length-2),g=stringUtil.upperCamel(c);if(r.primaryKey&&(s+=`    @Id
...
`;o.length>1&&r.primaryKey&&!r.autoIncrement&&(s+=`    @ManyToOne
