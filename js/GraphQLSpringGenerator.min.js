...
`;r.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`service/${o}Service.java`,content:g})}}),r}gereratePageUtilFile(){let e=`package ${this.packageName}.utils;
...
`);let g=`package ${this.packageName}.repository;
...
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`repository/${o}Repository.java`,content:g});let u=`package ${this.packageName}.repository;
...
`;let p=[];t.columns.forEach(r=>{let a=r.name,i=stringUtil.camelize(a),o=r.type,l=this.getJavaType(o),c=i.substring(0,i.length-2),d=stringUtil.upperCamel(c);if(r.primaryKey&&(s+=`    @Id
