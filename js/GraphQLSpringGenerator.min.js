...
`;return[{name:"src/main/resources/application.properties",content:r}]}generateRequestInterceptorFile(){let e=`package ${this.packageName}.config;
...
}`;return[{name:this.createSourceDirectoryFromArtefact(this.packageName)+"config/CorsConfig.java",content:e}]}generateControllerFiles(){let e=this.model.entities,t=[];return e.forEach(e=>{let r=e.name,a=stringUtil.camelize(r),i=stringUtil.upperCamel(a),o=this.getPrimaryKeys(e);if(o.length>0){let n=o.map(e=>`@Argument ${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),s=o.map(e=>stringUtil.camelize(e.name)).join(", "),p=`package ${this.packageName}.controller;
...
public class ${i}Controller {
    ...
    @QueryMapping
    public ${i} get${i}(${n}, DataFetchingEnvironment env) {
    ...
    @MutationMapping
    public Boolean delete${i}(${n}, DataFetchingEnvironment env) {
        ${a}Service.delete${i}(${s});
        return true;
    }
}
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`controller/${i}Controller.java`,content:p})}}),t}generateServiceFiles(){let e=this,t=this.model.entities,r=[];return t.forEach(t=>{let a=t.name,i=stringUtil.camelize(a),o=stringUtil.upperCamel(i),n=this.getPrimaryKeys(t),s="";if(t.columns.forEach(t=>{let r=e.getDataType(t.type),a=e.getFilterType(r),i=stringUtil.camelize(t.name);s+=`		this.fetchProperties.add("${i}", "${i}", "${r}", "${a}");\r
`}),n.length>0){let p=n.map(e=>`${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),l=n.map(e=>stringUtil.camelize(e.name)).join(", "),c='"'+n.map(e=>stringUtil.camelize(e.name)).join('", "')+'"',g=n.map(e=>stringUtil.upperCamel(stringUtil.camelize(e.name))).join("And"),d=[];n.forEach(e=>{let t=stringUtil.upperCamel(stringUtil.camelize(e.name));d.push(`input.get${t}() == null`)});let m=`package ${this.packageName}.service;
...
${s}
     ...
     */
    public ${o} get${o}(${p}) {
        return ${i}Repository.findOneBy${g}(${l});
    }
    ...
    public ${o} create${o}(${o}Input input) {
        ...
        return ${i}Repository.findOneBy${g}(
    ...
    public void delete${o}(${p}) {
        ${i}InputRepository.deleteBy${g}(${l});
    }
}
`;r.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`service/${o}Service.java`,content:m})}}),r}gereratePageUtilFile(){let e=`package ${this.packageName}.utils;
...
`;return[{name:this.createSourceDirectoryFromArtefact(this.packageName)+"utils/DataOrder.java",content:e}]}getNestedRelations(e,t,r="",a=2){if(a<=0)return[];let i=[],o=this.getPrimaryKeys(t).length>1;return t.columns.forEach(n=>{if(this.isForeignKey(e,t,n)&&!o){let s=stringUtil.camelize(n.name.replace(/_id$/i,"")),p=r?`${r}.${s}`:s;i.push(p);let l=e.find(e=>e.name.toLowerCase()===s.toLowerCase());l&&i.push(...this.getNestedRelations(e,l,p,a-1))}}),t.columns.forEach(t=>{if(o){let n=stringUtil.camelize(t.name.replace(/_id$/i,"")),s=r?`${r}.${n}`:n;i.push(s);let p=e.find(e=>e.name.toLowerCase()===n.toLowerCase());p&&i.push(...this.getNestedRelations(e,p,s,a-1))}}),i}generateConnectionFiles(){let e=this.model.entities,t=[];return e.forEach(e=>{let r=stringUtil.camelize(e.name),a=stringUtil.upperCamel(r),i=`package ${this.packageName}.output;
...
`;t.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`output/${a}Connection.java`,content:i})}),t}generateRepositoryFiles(){let e=this.model.entities,t=[],r=this;return e.forEach(a=>{let i=stringUtil.camelize(a.name),o=stringUtil.upperCamel(i),n=this.getPrimaryKeys(a);if(n.length>0){let s=n.length>1?`${o}Id`:this.getJavaType(n[0].type),p=n.map(e=>stringUtil.upperCamel(stringUtil.camelize(e.name))).join("And"),l=n.map(e=>`${this.getJavaType(e.type)} ${stringUtil.camelize(e.name)}`).join(", "),c="";["String","Long","Integer","Double","Float","Boolean","Character","Byte","Short"].includes(s)||s.includes(".")||(c=`import ${this.packageName}.entity.${s};
`);let g=this.getNestedRelations(e,a,"",r.maxRelationDepth||3),d="";(g.length>0||n.length>1)&&(d=`    @EntityGraph(attributePaths = { ${g.map(e=>`"${e}"`).join(", ")} })
...
${d}    Page<${o}> findAll(Specification<${o}> specification, Pageable pageable);
}
...
`;let p=[];t.columns.forEach(r=>{let a=r.name,i=stringUtil.camelize(a),o=r.type,l=this.getJavaType(o),c=i.substring(0,i.length-2),g=stringUtil.upperCamel(c);if(r.primaryKey&&(s+=`    @Id
...
`,p.push({name:c,type:g})}else s+=`    @Column(name = "${a}")
    private ${l} ${i};
...
`,t.columns.forEach(e=>{let t=e.name,r=stringUtil.camelize(t),a=e.type,i=this.getJavaType(a);e.primaryKey&&(l+=`    @Id
...
`;r.forEach(e=>{let t=stringUtil.camelize(e.name),r=this.getJavaType(e.type);e.primaryKey&&(i+="",e.autoIncrement&&(i+=`    @GeneratedValue(strategy = GenerationType.IDENTITY)
`)),i+=`    private ${r} ${t};
...
`}),i+=`    @Override
    public boolean equals(Object o) {
        ...
        ${a} that = (${a}) o;
        return ${r.map(e=>`Objects.equals(${stringUtil.camelize(e.name)}, that.${stringUtil.camelize(e.name)})`).join(" && ")};
    }

    @Override
    public int hashCode() {
        return Objects.hash(${r.map(e=>stringUtil.camelize(e.name)).join(", ")});
    }

`,i+=`}
`,e.push({name:this.createSourceDirectoryFromArtefact(this.packageName)+`entity/${a}.java`,content:i})}}),e}generateGraphQlSchema(){return[{name:"src/main/resources/graphql/schema.graphqls",content:util.buildGraphQLSchema(this.model.entities,!1)}]}generatePageInfoFile(){let e=`package ${this.packageName}.utils;
